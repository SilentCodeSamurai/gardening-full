...
function ComboboxInput({
	...
	return (
		<ComboboxPrimitive.InputGroup
			...
			className={cn(inputGroupShellClassName, "w-auto", className)}
		>
			{startAdornment ? (
				<InputGroupAddon align="inline-start" className="h-full min-h-0 shrink-0 self-stretch py-0 pr-0 pl-0.5">
					{startAdornment}
				</InputGroupAddon>
			) : null}
			<ComboboxPrimitive.Input
				render={<InputGroupInput disabled={disabled} className="py-0 leading-none" />}
				{...props}
			/>
			<InputGroupAddon align="inline-end" className="h-full min-h-0 shrink-0 self-stretch py-0 pr-2 pl-0">
				{showTrigger && (
					<InputGroupButton
						size="icon-xs"
						variant="ghost"
...
function ComboboxList({ className, ...props }: ComboboxPrimitive.List.Props) {
	return (
		<ComboboxPrimitive.List
			...
			className={cn(
				"no-scrollbar max-h-[min(calc(--spacing(72)---spacing(9)),calc(var(--available-height)---spacing(9)))] scroll-py-0 overflow-y-auto overscroll-contain p-0 data-empty:p-0",
				className,
			)}
			{...props}
		/>
...
function ComboboxItem({ className, children, ...props }: ComboboxPrimitive.Item.Props) {
	return (
		<ComboboxPrimitive.Item
			...
			className={cn(
				"relative flex h-7 w-full shrink-0 cursor-default select-none items-center gap-1.5 rounded-md py-0 pr-6 pl-0.5 text-xs/relaxed leading-none outline-hidden data-disabled:pointer-events-none data-highlighted:bg-accent data-disabled:opacity-50 data-[variant=destructive]:data-highlighted:[&>*:not([data-slot=item-presentation-icon])]:text-destructive not-data-[variant=destructive]:data-highlighted:[&>*:not([data-slot=item-presentation-icon])]:text-accent-foreground [&_svg:not([class*='size-'])]:size-3 [&_svg]:pointer-events-none [&_svg]:shrink-0",
				className,
			)}
			{...props}
		>
			...
			<ComboboxPrimitive.ItemIndicator
				render={
					<span className="pointer-events-none absolute top-1/2 right-1.5 flex -translate-y-1/2 items-center justify-center" />
				}
			>
