	...
	if (isError || !data) {
		return (
			<div className="text-destructive text-sm">
				{`${m["collections.speciesCategory.title"]()} ${m["common.notFound"]()}`}
			</div>
		);
	}
					...
					<dl className="grid gap-x-4 gap-y-3 text-sm sm:grid-cols-[minmax(9rem,auto)_1fr]">
						<div className="contents">
							...
							<dd className="wrap-break-word min-w-0">
								{data.isDefault ? m["common.yes"]() : m["common.no"]()}
							</dd>
						</div>
						<div className="contents">
						...
						<div className="contents sm:col-span-2">
							...
							<dd className="min-w-0">
								<div className="flex flex-col gap-2">
									...
									>
										{m["components.detail.link.speciesInCategory"]()}
									</Link>
									<Link
										...
										className="inline-flex w-fit text-primary underline-offset-4 hover:underline"
									>
										{m["components.detail.link.cultivarsInCategory"]()}
									</Link>
									<Link
										to="/plants"
										...
										className="inline-flex w-fit text-primary underline-offset-4 hover:underline"
									>
										{m["components.detail.link.plantsInCategory"]()}
									</Link>
								</div>
							</dd>
						</div>
